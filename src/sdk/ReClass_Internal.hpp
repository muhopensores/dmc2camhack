...
public:
	...
	Vector4f curPos; //0x0010
	Vector3f displace; //0x0020
	char pad_002C[160]; //0x002C
	float N00000380; //0x00CC
	Vector3f unkVec3; //0x00D0
	float N00000381; //0x00DC
	Vector4f posAgain; //0x00E0
	char pad_00F0[80]; //0x00F0
	uint16_t hp; //0x0140
	uint16_t N00000449; //0x0142
	uint16_t maxHp; //0x0144
	uint16_t armour; //0x0146
	uint16_t maxArmour; //0x0148
	char pad_014A[960]; //0x014A
}; //Size: 0x050A
...
public:
	char pad_0000[16]; //0x0000
	Vector4f plrCoords; //0x0010
	char pad_0020[1224]; //0x0020
	class emStruct01 *emStruct01Ptr; //0x04E8
}; //Size: 0x04F0



class cameraStructSomething
{
public:
	Vector4f position; //0x0000
	Vector4f lookAt; //0x0010
	char pad_0020[24]; //0x0020
	int32_t pad00; //0x0038
	Matrix3x3f notMatrix; //0x003C
	char pad_0060[32]; //0x0060
	float unkFloat; //0x0080
	uint32_t pad01; //0x0084
	float fov; //0x0088
	uint32_t N0000007D; //0x008C
	uint16_t N00000B61; //0x0090
	uint16_t controlDir; //0x0092
	char pad_0094[36]; //0x0094
	float N00000070; //0x00B8
	float N0000008D; //0x00BC
	float N00000071; //0x00C0
	float N00000090; //0x00C4
	char pad_00C8[56]; //0x00C8
	uint8_t subclassStartUnkUint8; //0x0100
	uint8_t unkUint8_1; //0x0101
	uint8_t unkUint8_2; //0x0102
	uint8_t unkUint8_3; //0x0103
	uint32_t pad02; //0x0104
	uint32_t pad03; //0x0108
	int32_t pad04; //0x010C
	Vector4f someFltPos0; //0x0110
	Vector4f lkat_copy; //0x0120
	Vector4f pos_copy; //0x0130
	Vector4f someFltPos3MultZero; //0x0140
	float N000002CD; //0x0150
	float anotherFoVprob; //0x0154
	float N00000096; //0x0158
	float N000002B6; //0x015C
	char pad_0160[192]; //0x0160
	uint8_t anotherCameraType; //0x0220
	uint8_t somethingCameraRelatedAlso; //0x0221
	uint16_t pad05; //0x0222
	uint32_t pad06; //0x0224
	uint8_t unkUint8_6; //0x0228
	uint16_t N00000B9C; //0x0229
	uint16_t N00000BAA; //0x022B
	char pad_022D[3]; //0x022D
	Vector4f N00000BB2; //0x0230
	Vector4f N00000BB3; //0x0240
	Vector4f N00000BAF; //0x0250
	char pad_0260[96]; //0x0260
	Vector4f N000000C0; //0x02C0
	Vector4f N000000C1; //0x02D0
	char pad_02E0[32]; //0x02E0
	float fovProbAgain; //0x0300
	char pad_0304[60]; //0x0304
	uint8_t unkUint8_7; //0x0340
	uint8_t unkUint8_8; //0x0341
	uint8_t unkUint8_9; //0x0342
	uint32_t pad07; //0x0343
	uint8_t unkUint8_10; //0x0347
	uint32_t unkUint32; //0x0348
	float unkFloat01; //0x034C
	Vector4f N00000BC4; //0x0350
	Vector4f N00000BBF; //0x0360
	Vector4f N00000BC0; //0x0370
	char pad_0380[224]; //0x0380
	uint8_t unkUint8_11; //0x0460
	uint8_t unkUint8_12; //0x0461
	char pad_0462[6]; //0x0462
	uint8_t unkUint8_13; //0x0468
	char pad_0469[279]; //0x0469
	uint8_t unkUint8_14; //0x0580
	uint8_t unkUint8_15; //0x0581
	char pad_0582[570]; //0x0582
	int32_t N00000171; //0x07BC
	uint8_t fovTrip; //0x07C0
	uint8_t cameraByte; //0x07C1
	uint8_t switchCamByte; //0x07C2
	uint8_t someOtherByte; //0x07C3
}; //Size: 0x07C4
...
public:
	...
	uint8_t lX; //0x0002
	uint8_t lY; //0x0003
	uint8_t dpadRight; //0x0004
	uint8_t dpadLeft; //0x0005
	uint8_t dpadUp; //0x0006
	uint8_t dpadDown; //0x0007
	uint8_t triangle; //0x0008
	uint8_t l1; //0x0009
	uint8_t circle; //0x000A
	uint8_t square; //0x000B
	uint8_t cross; //0x000C
	uint8_t r1; //0x000D
	uint8_t l2; //0x000E
	uint8_t r2; //0x000F
	uint8_t N000004F9; //0x0010
	uint8_t rDir; //0x0011
	char pad_0012[7]; //0x0012
	uint8_t lDir; //0x0019
	char pad_001A[103]; //0x001A
}; //Size: 0x0081
